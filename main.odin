...
render_screen :: proc(window: glfw.WindowHandle, vao: VAO) {
	...
	rotation_mat := [9]f32{m.cos(theta), m.sin(-theta), 0, m.sin(theta), m.cos(theta), 0, 0, 0, 1}
	...
	gl.UniformMatrix3fv(
		gl.GetUniformLocation(global_shader, "rotation_mat"),
		1,
		gl.TRUE,
		rotation_ptr,
	)
	gl.UniformMatrix3fv(
		gl.GetUniformLocation(global_shader, "camera_to_x_mat"),
		1,
		gl.TRUE,
		camera_to_x_ptr,
	)
	gl.UniformMatrix4fv(
		gl.GetUniformLocation(global_shader, "translation_mat"),
		1,
		gl.TRUE,
		translation_ptr,
	)
	gl.UniformMatrix4fv(
		gl.GetUniformLocation(global_shader, "perspective_mat"),
		1,
		gl.TRUE,
		perspective_ptr,
	)
